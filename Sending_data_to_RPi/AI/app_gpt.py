...
def track_heads(all_box_coords, ht: HeadTracker):
    ...
    for (objectID, centroid) in objects.items():
        if objectID not in kalman_filters:
            ...
            predicted = (centroid[0], centroid[1])
...
def draw_bounding_box(box, img):
    ...
    org = [x1, y1]
    cv2.putText(img, f"Head {confidence}", org, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
...
while True:
    ...
    results = model.predict(img, stream=True, iou=0.4, conf=0.5, verbose=False) # Make prediction with model
